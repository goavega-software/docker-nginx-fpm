...
# setup env variables and php 8.2 redis
...
#| supervisor            |
#|-----------------------|
RUN apk add --no-cache freetype-dev libjpeg-turbo-dev libpng-dev openrc openssh supervisor \
    && docker-php-ext-configure gd --with-freetype --with-jpeg \
    && docker-php-ext-install -j$(nproc) gd \
    && apk del freetype-dev libjpeg-turbo-dev libpng-dev util-linux-dev \
    && rc-update add sshd
...
RUN set -x \
    ...
    *) \
    ...
    && su nobody -s /bin/sh -c "export HOME=${tempDir} && cd ${tempDir} && hg clone https://hg.nginx.org/pkg-oss && cd pkg-oss && hg up ${NGINX_VERSION}-${PKG_RELEASE} && cd alpine && make all && apk index -o ${tempDir}/packages/alpine/${apkArch}/APKINDEX.tar.gz ${tempDir}/packages/alpine/${apkArch}/*.apk && abuild-sign -k ${tempDir}/.abuild/abuild-key.rsa ${tempDir}/packages/alpine/${apkArch}/APKINDEX.tar.gz" \
...
# Nginx and php-fpm config (docker nginx runs nginx user - change fpm to use same user)
RUN set -ex && \
    ...
    sed -i -e "s/listen = 127.0.0.1:9000/listen = \/run\/php-fpm.sock/g" ${FPM_POOL_CONF} && \
...
# cleanup default docker php-fpm conf
